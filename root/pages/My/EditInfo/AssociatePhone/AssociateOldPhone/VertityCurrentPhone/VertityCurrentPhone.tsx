...
const VertityCurrentPhone: FunctionComponent = (props) => {
    ...
    return (
        <View style={[styles.container_clear, { paddingHorizontal: pxToDp(30), paddingTop: headerHeight }]}>
            ...
            <IDBitTabBg style={{ height: pxToDp(88), paddingHorizontal: pxToDp(20), borderRadius: pxToDp(12), marginTop: pxToDp(12), justifyContent: "center" }}>
                {/* <Text style={{ fontSize: pxToSp(28), color: '#B8B8B8' }}>13066660000</Text> */}
                <Text style={{ fontSize: pxToSp(28), color: '#fff' }}>{imUserInfo?.phoneNumber}</Text>
            </IDBitTabBg>
            <IDBitTabBg style={{ height: pxToDp(88), paddingLeft: pxToDp(20), borderRadius: pxToDp(12), marginTop: pxToDp(12), flexDirection: 'row', justifyContent: "space-between", alignItems: "center" }}>
                <TextInput value={vertity} onChangeText={(text: any) => { setvertity(text) }} style={{ width: pxToDp(200), color: '#fff' }} placeholderTextColor={'#575C64'} placeholder={'输入验证码'}></TextInput>
                ...
                <CountDownButton
                    ...
                    onClick={async (shouldStartCountting: Function) => {
                        if (checkPhone(imUserInfo?.phoneNumber) == false) {
                            toast('手机号格式错误！')
                            return
                        }
                        await UserService.sendSmsApi(imUserInfo?.phoneNumber)
                        //随机模拟发送验证码成功或失败
            ...
            <IDBitBtn text="下一步" containerStyle={{ bottom: useSafeAreaInsets().bottom + pxToDp(60), height: pxToDp(88), alignSelf: "center", position: 'absolute', paddingHorizontal: pxToDp(60) - UIELEMENTS.PADDING_HORIZONTAL }} onPress={async () => {
                if (checkPhone(imUserInfo?.phoneNumber) == false) {
                    toast('手机号格式错误！')
                    return
                }
                setisShow(true)
                const resp = await UserService.updateTelephoneInfoApi(vertity, imUserInfo?.phoneNumber, '', '86')
                setisShow(false)
                if (resp.errCode == 0) {
                        setTimeout(() => {
                            Navigate.navigate('SetNewPhone')
                        }, 1000);
                }
                else toast('失败:' + JSON.stringify(resp.errMsg))
...
export default VertityCurrentPhone;
