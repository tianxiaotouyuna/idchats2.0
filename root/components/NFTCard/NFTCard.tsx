...
const [isShow, setisShow] = useState(false);
  ...
  const handleUpdateAvatar = async (item:any)=>{
    ...
    console.log('kkkkkkkk'+JSON.stringify(info))
  ...
  const item_one_collections = (item: any, index: number) => {
    return (
      item!='zhanwei'?<Ripple onPress={() => {cardStyle==CardStyle.SETAVATAR_STYLE? handleUpdateAvatar(item):Navigate.navigate('NftDetail',{nftInfo:item}) }} rippleColor={UIELEMENTS.DEFAULT_HEADER_COLOR} key={item?.name+index}>
